...
export function emitExpr(ctx: EmitContext, expr: HIRExpr): string {
  const m = ctx.m;
  ...
  switch (expr.kind) {
    ...
    case "literal_i64":
      return m.constInt(m.i64, expr.value, false);
    case "literal_i1":
      return m.constInt(m.i1, expr.value ? 1 : 0, false);
    case "literal_string":
...
function emitAllocArray(ctx: EmitContext, expr: HIRExpr & { kind: "alloc_array" }): string {
  const m = ctx.m;
  ...
  const arr = m.buildCall(newDecl.fnType, newDecl.fn, [m.constInt(m.i32, capacity, false)], "arr");
...
function emitAllocArraySpread(
  ...
  const arr = m.buildCall(newDecl.fnType, newDecl.fn, [m.constInt(m.i32, 4, false)], "arr");
...
function emitAllocStruct(ctx: EmitContext, expr: HIRExpr & { kind: "alloc_struct" }): string {
  const m = ctx.m;
  ...
  for (let i = 0; i < expr.fields.length; i++) {
    ...
    const fieldPtr = m.buildGEP(
      structInfo.llvmType,
      raw,
      [m.constInt(m.i32, 0, false), m.constInt(m.i32, i, false)],
      "",
    );
  ...
  if (ifaceInfo) {
    ...
    const dataSlot = m.buildGEP(ifaceInfo.fatType, fat, [m.constInt(m.i32, 0, false), m.constInt(m.i32, 0, false)], "");
    m.buildStore(raw, dataSlot);
    const vtableSlot = m.buildGEP(ifaceInfo.fatType, fat, [m.constInt(m.i32, 0, false), m.constInt(m.i32, 1, false)], "");
...
function dynObjSetFunc(ctx: EmitContext, valueType: HIRType): { fn: string; fnType: string } | null {
...
function emitFieldGet(ctx: EmitContext, expr: HIRExpr & { kind: "field_get" }): string {
  const m = ctx.m;
  ...
  if (ifaceInfo) {
    ...
    const dataSlot = m.buildGEP(ifaceInfo.fatType, fatPtr, [m.constInt(m.i32, 0, false), m.constInt(m.i32, 0, false)], "");
    const dataPtr = m.buildLoad(m.ptr, dataSlot, "data");
    const fieldPtr = m.buildGEP(
      ifaceInfo.layoutType,
      dataPtr,
      [m.constInt(m.i32, 0, false), m.constInt(m.i32, expr.index, false)],
      "",
    );
...
function emitFieldSet(ctx: EmitContext, expr: HIRExpr & { kind: "field_set" }): string {
  const m = ctx.m;
  const obj = emitExpr(ctx, expr.object);
  const val = emitExpr(ctx, expr.value);
...
function emitWrapInterface(ctx: EmitContext, expr: HIRExpr & { kind: "wrap_interface" }): string {
  const m = ctx.m;
...
function emitVtableCall(ctx: EmitContext, expr: HIRExpr & { kind: "vtable_call" }): string {
  const m = ctx.m;
  const fatPtr = emitExpr(ctx, expr.object);
  ...
  const dataSlot = m.buildGEP(ifaceInfo.fatType, fatPtr, [m.constInt(m.i32, 0, false), m.constInt(m.i32, 0, false)], "");
  const dataPtr = m.buildLoad(m.ptr, dataSlot, "data");
  const vtableSlot = m.buildGEP(ifaceInfo.fatType, fatPtr, [m.constInt(m.i32, 0, false), m.constInt(m.i32, 1, false)], "");
  ...
  const fnPtrPtr = m.buildGEP(
    vtableArrTy,
    vtablePtr,
    [m.constInt(m.i32, 0, false), m.constInt(m.i32, expr.methodIndex, false)],
    "",
  );
...
function emitIndexGet(ctx: EmitContext, expr: HIRExpr & { kind: "index_get" }): string {
  const m = ctx.m;
  ...
  const elemType =
    expr.array.type.kind === "array" ? (expr.array.type as string).element : { kind: "f64" };
...
function emitIndexSet(ctx: EmitContext, expr: HIRExpr & { kind: "index_set" }): string {
  const m = ctx.m;
  const arr = emitExpr(ctx, expr.array);
  let idx = emitExpr(ctx, expr.index);
  if (expr.index.type.kind === "i64") {
...
function emitCall(ctx: EmitContext, expr: HIRExpr & { kind: "call" }): string {
  const m = ctx.m;
...
function emitBoxedBinary(ctx: EmitContext, expr: HIRExpr & { kind: "binary" }): string {
  const m = ctx.m;
...
function emitStringCompare(ctx: EmitContext, expr: HIRExpr & { kind: "binary" }): string {
  const m = ctx.m;
  const left = emitExpr(ctx, expr.left);
  const right = emitExpr(ctx, expr.right);
  ...
  if (expr.op === "str_eq") {
    return m.buildICmp(LLVMIntEQ, result, m.constInt(m.i32, 0, false), "");
  }
  return m.buildICmp(LLVMIntNE, result, m.constInt(m.i32, 0, false), "");
}
...
function emitShortCircuit(ctx: EmitContext, expr: HIRExpr & { kind: "binary" }): string {
  ...
  const leftCoerced = isBoolean
    ? (expr.op === "or" ? m.constInt(m.i1, 1, false) : m.constInt(m.i1, 0, false))
  ...
  if (isBoolean) {
    ...
    if (expr.op === "and") {
      m.addIncoming(phi, [rightCond, m.constInt(m.i1, 0, false)], [rhsEndBlock, leftBlock]);
    } else {
      m.addIncoming(phi, [rightCond, m.constInt(m.i1, 1, false)], [rhsEndBlock, leftBlock]);
    }
    return phi;
  }
...
function emitUnary(ctx: EmitContext, expr: HIRExpr & { kind: "unary" }): string {
  ...
  if (expr.operand.type.kind === "boxed") {
    switch (expr.op) {
      ...
      case "not": {
        ...
        return m.buildICmp(LLVMIntEQ, truthy, m.constInt(m.i32, 0, false), "");
      }
      default:
  ...
  switch (expr.op) {
    ...
    case "not": {
      ...
      return m.buildXor(boolVal, m.constInt(m.i1, 1, false), "");
    }
    case "bit_not":
...
function emitRuntimeCall(ctx: EmitContext, expr: HIRExpr & { kind: "runtime_call" }): string {
  ...
  if (
    ...
    return m.constInt(m.i32, 0, false);
  }
  ...
  if (bridgeFn) {
    ...
    if (expr.returnType.kind === "i64") {
      ...
      result = m.buildICmp(LLVMIntNE, result, m.constInt(m.i32, 0, false), "");
    }
    return result;
  }
...
function emitPrintValueStderr(ctx: EmitContext, arg: HIRExpr, val: string, isLast: boolean): void {
  const m = ctx.m;
  if (arg.type.kind === "i8ptr") {
...
function emitToString(ctx: EmitContext, arg: HIRExpr, val: string): string {
  ...
  const malloc = ctx.getDeclaredFunction("malloc")!;
  const buf = m.buildCall(malloc.fnType, malloc.fn, [m.constInt(m.i64, 32, false)], "buf");
...
function emitStringConcat(ctx: EmitContext, expr: HIRExpr & { kind: "runtime_call" }): string {
  ...
  const totalLen1 = m.buildAdd(totalLen, m.constInt(m.i64, 1, false), "");
...
function emitMathCall(ctx: EmitContext, expr: HIRExpr & { kind: "runtime_call" }): string {
  ...
  if (expr.func === "cs_math_clz32") {
    ...
    const result = m.buildCall(ctlz.fnType, ctlz.fn, [i32Val, m.constInt(m.i1, 0, false)], "");
...
function emitMakeClosure(ctx: EmitContext, expr: HIRExpr & { kind: "make_closure" }): string {
  const m = ctx.m;
  ...
  const closureSize = m.constInt(m.i64, 16, false);
  const rawPtr = m.buildCall(mallocDecl.fnType, mallocDecl.fn, [closureSize], "closure_raw");

  const fnSlot = m.buildGEP(m.i8, rawPtr, [m.constInt(m.i64, 0, false)], "fn_slot");
  m.buildStore(fnPtr, fnSlot);
  const envSlot = m.buildGEP(m.i8, rawPtr, [m.constInt(m.i64, 8, false)], "env_slot");
  m.buildStore(envPtr, envSlot);

  return rawPtr;
}

function emitCallClosure(ctx: EmitContext, expr: HIRExpr & { kind: "call_closure" }): string {
  const m = ctx.m;
  const closurePtr = emitExpr(ctx, expr.callee);

  const fnSlot = m.buildGEP(m.i8, closurePtr, [m.constInt(m.i64, 0, false)], "fn_slot");
  const fnPtr = m.buildLoad(m.ptr, fnSlot, "fn_ptr");
  const envSlot = m.buildGEP(m.i8, closurePtr, [m.constInt(m.i64, 8, false)], "env_slot");
...
function emitArrayHof(
  ...
  const fnSlot = m.buildGEP(m.i8, closurePtr, [m.constInt(m.i64, 0, false)], "hof_fn_slot");
  const fnPtr = m.buildLoad(m.ptr, fnSlot, "hof_fn_ptr");
  const envSlot = m.buildGEP(m.i8, closurePtr, [m.constInt(m.i64, 8, false)], "hof_env_slot");
...
function emitPromiseStatic(
  ...
  const arr = m.buildCall(
    objNewDecl.fnType,
    objNewDecl.fn,
    [m.constInt(m.i32, Math.max(expr.promises.length, 4), false)],
    "promises_arr",
  );
...
function emitPromiseAllSettled(
  ...
  for (let i = 0; i < expr.promises.length; i++) {
    const promisePtr = m.buildCall(
      objGetDecl.fnType,
      objGetDecl.fn,
      [promisesArr, m.constInt(m.i32, i, false)],
      "p",
    );
    ...
    const statusPtr = m.buildGEP(
      structInfo.llvmType,
      obj,
      [m.constInt(m.i32, 0, false), m.constInt(m.i32, 0, false)],
      "",
    );
    ...
    const valuePtr = m.buildGEP(
      structInfo.llvmType,
      obj,
      [m.constInt(m.i32, 0, false), m.constInt(m.i32, 1, false)],
      "",
    );
...
function emitNullishCoalesce(ctx: EmitContext, expr: HIRExpr & { kind: "nullish_coalesce" }): string {
  const m = ctx.m;
  const fn = ctx.getCurrentFn();
...
export function ensureI1(ctx: EmitContext, val: string, hirType: HIRType): string {
  const m = ctx.m;
  switch (hirType.kind) {
    case "i1":
      return val;
    ...
    case "i64":
      return m.buildICmp(LLVMIntNE, val, m.constInt(m.i64, 0, false), "");
    case "i8ptr":
    case "dynobj":
    case "dynarray":
    case "map":
