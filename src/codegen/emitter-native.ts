...
import { LLVMModule, LLVMIntEQ, LLVMIntNE, LLVMRealOEQ, LLVMPrivateLinkage } from "./llvm-native.js";
import {
  EmitContext,
  CaptureMap,
  CaptureEnvEntry,
...
} from "./emit-context-native.js";
import { emitExpr, ensureI1 } from "./emit-expr-native.js";

export type { CaptureMap, CaptureEnvEntry } from "./emit-context-native.js";
    ...
    for (const outerFn of mod.functions) {
      ...
      if (overlap.length > 0) {
        ...
        for (const cid of overlap) {
          const alreadyIn = existing.find((e) => e.id === cid);
          if (!alreadyIn) {
...
function scanExprForClosures(expr: HIRExpr, names: Set<string>): void {
  switch (expr.kind) {
    case "make_closure": names.add(expr.funcName); break;
    case "box": case "unbox": case "narrow_i64": case "widen_f64": case "wrap_interface":
      scanExprForClosures(expr.value, names); break;
    case "binary": scanExprForClosures(expr.left, names); scanExprForClosures(expr.right, names); break;
    case "unary": scanExprForClosures(expr.operand, names); break;
    case "call": case "runtime_call":
      for (const a of expr.args) scanExprForClosures(a, names); break;
    case "call_indirect": case "call_closure":
      scanExprForClosures(expr.callee, names);
      for (const a of expr.args) scanExprForClosures(a, names); break;
    case "vtable_call":
      scanExprForClosures(expr.object, names);
      for (const a of expr.args) scanExprForClosures(a, names); break;
    case "field_get": scanExprForClosures(expr.object, names); break;
    case "field_set": scanExprForClosures(expr.object, names); scanExprForClosures(expr.value, names); break;
    case "index_get": scanExprForClosures(expr.array, names); scanExprForClosures(expr.index, names); break;
    case "index_set":
      scanExprForClosures(expr.array, names);
      scanExprForClosures(expr.index, names);
      scanExprForClosures(expr.value, names); break;
    case "local_set": case "global_set": scanExprForClosures(expr.value, names); break;
    case "alloc_struct": for (const f of expr.fields) scanExprForClosures(f, names); break;
    case "alloc_array": for (const v of expr.initialValues) scanExprForClosures(v, names); break;
    case "alloc_array_spread": for (const e of expr.elements) scanExprForClosures(e.value, names); break;
    case "alloc_map":
      for (const e of expr.entries) { scanExprForClosures(e.key, names); scanExprForClosures(e.value, names); }
      if (expr.spreadSource) scanExprForClosures(expr.spreadSource, names); break;
    case "alloc_set": for (const e of expr.elements) scanExprForClosures(e, names); break;
    case "alloc_dynobj":
      for (const p of expr.props) scanExprForClosures(p.value, names);
      if (expr.spreadSource) scanExprForClosures(expr.spreadSource, names); break;
    case "conditional":
      scanExprForClosures(expr.condition, names);
      scanExprForClosures(expr.then, names);
      scanExprForClosures(expr.else, names); break;
    case "array_hof":
      scanExprForClosures(expr.array, names);
      scanExprForClosures(expr.callback, names);
      if (expr.initialValue) scanExprForClosures(expr.initialValue, names); break;
    case "nullish_coalesce": scanExprForClosures(expr.left, names); scanExprForClosures(expr.right, names); break;
    case "await": scanExprForClosures(expr.value, names); break;
    case "promise_static": for (const p of expr.promises) scanExprForClosures(p, names); break;
    default: break;
  }
}

function scanStmtsForClosures(stmts: HIRStmt[], names: Set<string>): void {
  for (const s of stmts) {
    if (s.kind === "expr") scanExprForClosures(s.expr, names);
    if (s.kind === "return" && s.value) scanExprForClosures(s.value, names);
    if (s.kind === "let" && s.init) scanExprForClosures(s.init, names);
    if (s.kind === "if") {
      scanExprForClosures(s.condition, names);
      scanStmtsForClosures(s.then, names);
      if (s.else) scanStmtsForClosures(s.else, names);
    }
    if (s.kind === "while") {
      scanExprForClosures(s.condition, names);
      scanStmtsForClosures(s.body, names);
    }
    if (s.kind === "for") scanStmtsForClosures(s.body, names);
  }
}

function findClosureFuncNames(mod: HIRModule, names: Set<string>): void {
  for (const fn of mod.functions) scanStmtsForClosures(fn.body, names);
  scanStmtsForClosures(mod.init, names);
}
...
function declareExterns(ctx: EmitContext): void {
  ...
  for (const pair1 of mathIntrinsics1) {
    const fn = m.addFunction(pair1[0], math1Type);
    ctx.declareMathIntrinsic(pair1[1], fn, math1Type);
  }
  ...
  for (const lm1 of libm1) {
    const fn = m.addFunction(lm1[0], math1Type);
    ctx.declareMathIntrinsic(lm1[1], fn, math1Type);
  }

  const mathIntrinsics2: string[][] = [
  ...
  for (const pair2 of mathIntrinsics2) {
    const fn = m.addFunction(pair2[0], math2Type);
    ctx.declareMathIntrinsic(pair2[1], fn, math2Type);
  }
  ...
  for (const lm2 of libm2) {
    const fn = m.addFunction(lm2[0], math2Type);
    ctx.declareMathIntrinsic(lm2[1], fn, math2Type);
  }
  ...
  const bridgeFns: [string, string, string[]][] = [
  ...
  for (const bf of bridgeFns) {
    const bfName = bf[0] as string;
    const bfRet = bf[1] as string;
    const bfParams = bf[2] as string[];
    const fnType = m.functionType(bfRet, bfParams);
    const fn = m.addFunction(bfName, fnType);
    ctx.declareFunction(bfName, fn, fnType);
  }
}
...
function emitFunction(
  ...
  if (isClosure) {
    ...
    if (captureInfo) {
      ...
      const envSize = m.constInt(m.i64, captureInfo.length * 8, false);
...
function emitMain(ctx: EmitContext, mod: HIRModule): void {
  ...
  m.buildRet(m.constInt(m.i32, 0, false));
}
...
function emitStmt(ctx: EmitContext, stmt: HIRStmt): void {
  ...
  switch (stmt.kind) {
    ...
    default:
      throw new Error(`unhandled statement kind: ${(stmt as string).kind}`);
  }
}
  ...
  for (let i = 0; i < stmt.cases.length; i++) {
    ...
    if (c.test) {
      ...
      if (discType.kind === "f64" && c.test.type.kind === "i64") {
        ...
        cmp = m.buildICmp(LLVMIntEQ, result, m.constInt(m.i32, 0, false), "");
      } else {
...
function emitTry(ctx: EmitContext, stmt: HIRStmt & { kind: "try" }): void {
  ...
  const isException = m.buildICmp(LLVMIntNE, sjResult, m.constInt(m.i32, 0, false), "is_exc");
