...
export function generateArraySort(
  ...
  if (expr.args.length === 1) {
    return generateSortWithCompareFn(gen, expr, params, arrayPtr, isStringArray);
  }
...
function generateSortWithCompareFn(
  gen: ArraySortContext,
  expr: MethodCallNode,
  params: string[],
  arrayPtr: string,
  ...
  if (isStringArray) {
    return generateStringArraySortWithFn(gen, arrayPtr, compareFn);
  }
  return generateNumericArraySortWithFn(gen, arrayPtr, compareFn);
}

function generateNumericArraySortWithFn(
  gen: ArraySortContext,
  arrayPtr: string,
  compareFn: string,
): string {
...
function generateStringArraySortWithFn(
  gen: ArraySortContext,
  arrayPtr: string,
  compareFn: string,
): string {
  ...
  const cmpResult = gen.nextTemp();
  gen.emit(`${cmpResult} = call double @${compareFn}(i8* ${valA}, i8* ${valB})`);
  const shouldSwap = gen.nextTemp();
  gen.emit(`${shouldSwap} = fcmp ogt double ${cmpResult}, 0.0`);
  gen.emit(`br i1 ${shouldSwap}, label %${swapLabel}, label %${noSwapLabel}`);
