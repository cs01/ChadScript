...
export class CryptoGenerator {
  ...
  private generateHash(expr: MethodCallNode, params: string[], evpFunc: string): string {
    ...
    this.ctx.emit(`${initResult} = call i32 @EVP_DigestInit_ex(i8* ${mdCtx}, i8* ${evpMd}, i8* null)`);

    const updateResult = this.ctx.nextTemp();
    this.ctx.emit(`${updateResult} = call i32 @EVP_DigestUpdate(i8* ${mdCtx}, i8* ${inputPtr}, i64 ${inputLen})`);
    ...
    this.ctx.emit(`${finalResult} = call i32 @EVP_DigestFinal_ex(i8* ${mdCtx}, i8* ${hashBuf}, i32* ${hashLenI32Ptr})`);
  ...
  generateBytesToHexHelper(): string {
    let ir = '';
    ir += '@.hex_chars = private unnamed_addr constant [17 x i8] c"0123456789abcdef\\00", align 1\n\n';
    ...
    ir += '  %hi_char_ptr = getelementptr inbounds [17 x i8], [17 x i8]* @.hex_chars, i64 0, i64 %hi_i64\n';
    ir += '  %lo_char_ptr = getelementptr inbounds [17 x i8], [17 x i8]* @.hex_chars, i64 0, i64 %lo_i64\n';
