...
export class ResponseGenerator {
  ...
  generateText(responsePtr: string): string {
    ...
    this.ctx.emit(
      `${bodyFieldPtr} = getelementptr %__FetchResponse, %__FetchResponse* ${responsePtr}, i32 0, i32 2`,
    );
  ...
  generateJson(responsePtr: string): string {
    ...
    this.ctx.emit(
      `${result} = phi i8* [ ${errorPtr}, %${errorLabel} ], [ ${resultPtr}, %${successLabel} ]`,
    );
  ...
  private generateJsonParser(typeName: string, interfaceDef: InterfaceDefInfo): void {
    ...
    for (let fieldIndex = 0; fieldIndex < interfaceDef.properties.length; fieldIndex++) {
      ...
      parserIR +=
        `  ${fieldPtr} = getelementptr inbounds %${typeName}, %${typeName}* %struct_ptr, i32 0, i32 ${fieldIndex}` +
        "\n";
      if (propType === "string") {
        parserIR +=
          `  store i8* getelementptr inbounds ([1 x i8], [1 x i8]* @.empty_str, i64 0, i64 0), i8** ${fieldPtr}` +
          "\n";
    ...
    for (let fieldIndex = 0; fieldIndex < interfaceDef.properties.length; fieldIndex++) {
      ...
      parserIR +=
        `  %item_${fieldIndex} = call i8* @csyyjson_obj_get(i8* %json_root, i8* getelementptr inbounds ([${propName.length + 1} x i8], [${propName.length + 1} x i8]* ${fieldNameConst}, i64 0, i64 0))` +
        "\n";

      if (propType === "string") {
        parserIR +=
          `  %temp_str_${fieldIndex} = call i8* @csyyjson_get_str(i8* %item_${fieldIndex})` + "\n";
        parserIR +=
          `  %safe_str_${fieldIndex} = call i8* @__safe_string(i8* %temp_str_${fieldIndex})` + "\n";
        parserIR += `  %value_${fieldIndex} = call i8* @strdup(i8* %safe_str_${fieldIndex})` + "\n";
      } else if (propType === "number") {
        parserIR +=
          `  %value_${fieldIndex} = call double @csyyjson_get_num(i8* %item_${fieldIndex})` + "\n";
      } else if (propType === "boolean") {
        parserIR +=
          `  %value_${fieldIndex} = call double @csyyjson_get_num(i8* %item_${fieldIndex})` + "\n";
      }
      ...
      parserIR +=
        `  %field_ptr_${fieldIndex} = getelementptr inbounds %${typeName}, %${typeName}* %struct_ptr, i32 0, i32 ${fieldIndex}` +
        "\n";
      ...
      if (propType === "string") {
        ...
        parserIR +=
          `  store double %value_${fieldIndex}, double* %field_ptr_${fieldIndex}` + "\n\n";
      } else if (propType === "boolean") {
        parserIR +=
          `  store double %value_${fieldIndex}, double* %field_ptr_${fieldIndex}` + "\n\n";
      }
  ...
  generateStatus(responsePtr: string): string {
    ...
    this.ctx.emit(
      `${statusFieldPtr} = getelementptr %__FetchResponse, %__FetchResponse* ${responsePtr}, i32 0, i32 1`,
    );
  ...
  generateUrl(responsePtr: string): string {
    ...
    this.ctx.emit(
      `${urlFieldPtr} = getelementptr %__FetchResponse, %__FetchResponse* ${responsePtr}, i32 0, i32 3`,
    );
  ...
  generateHeaders(responsePtr: string): string {
    ...
    this.ctx.emit(
      `${headersFieldPtr} = getelementptr %__FetchResponse, %__FetchResponse* ${responsePtr}, i32 0, i32 4`,
    );
  ...
  generateRedirected(responsePtr: string): string {
    ...
    this.ctx.emit(
      `${redirFieldPtr} = getelementptr %__FetchResponse, %__FetchResponse* ${responsePtr}, i32 0, i32 5`,
    );
