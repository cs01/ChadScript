...
export function generateProcessExitInline(
...
export function handleProcessChdir(
  ctx: MethodCallGeneratorContext,
  expr: MethodCallNode,
  params: string[],
): string {
...
export function handleProcessWrite(
  ...
  if (isStderr) {
    ...
    if (isString || (arg as ExprBase).type === "string") {
      ...
      ctx.emit(
        `${temp} = call i32 (i8*, i8*, ...) @fprintf(i8* ${stderrPtr}, i8* getelementptr([3 x i8], [3 x i8]* @.str.strfmt_no_nl, i32 0, i32 0), i8* ${argValue})`,
      );
    } else {
      const temp = ctx.nextTemp();
      ctx.emit(
        `${temp} = call i32 (i8*, i8*, ...) @fprintf(i8* ${stderrPtr}, i8* getelementptr([6 x i8], [6 x i8]* @.str.numfmt_no_nl, i32 0, i32 0), double ${ctx.ensureDouble(argValue)})`,
      );
    }
    ...
    if (isString || (arg as ExprBase).type === "string") {
      const temp = ctx.nextTemp();
      ctx.emit(
        `${temp} = call i32 (i8*, ...) @printf(i8* getelementptr([3 x i8], [3 x i8]* @.str.strfmt_no_nl, i32 0, i32 0), i8* ${argValue})`,
      );
    } else {
      const temp = ctx.nextTemp();
      ctx.emit(
        `${temp} = call i32 (i8*, ...) @printf(i8* getelementptr([6 x i8], [6 x i8]* @.str.numfmt_no_nl, i32 0, i32 0), double ${ctx.ensureDouble(argValue)})`,
      );
    }
    const flushTemp = ctx.nextTemp();
