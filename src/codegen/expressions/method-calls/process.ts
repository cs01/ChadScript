import type { Expression, MethodCallNode, MemberAccessNode, VariableNode } from '../../../ast/types.js';
...
export function handleProcessUptime(ctx: MethodCallGeneratorContext): string {
    ...
    const result = ctx.nextTemp();
    ctx.emit(`${result} = fadd double ${secDouble}, ${nsecSec}`);
    return result;
}

export function handleProcessSyscallI32(ctx: MethodCallGeneratorContext, funcName: string): string {
    const i32Result = ctx.nextTemp();
    ctx.emit(`${i32Result} = call i32 ${funcName}()`);
    const doubleResult = ctx.nextTemp();
    ctx.emit(`${doubleResult} = sitofp i32 ${i32Result} to double`);
    return doubleResult;
}
