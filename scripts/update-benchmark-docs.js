...
function buildRaceData(json) {
  ...
  for (const key of raceOrder) {
    ...
    lines.push(`    items: [`);
    for (const item of items) {
      const parts = [`name: '${item.name}'`, `val: '${item.val}'`, `speed: ${item.speed}`, `color: '${item.color}'`];
      if (item.hero) parts.push(`hero: true`);
      lines.push(`      { ${parts.join(', ')} },`);
    }
    lines.push(`    ],`);
...
if (fs.existsSync(MD_PATH)) {
  ...
  const startMarker = '**ChadScript delivers compiled-language performance with TypeScript syntax:**\n\n';
  const endMarker = '\n\nReproduce:';
  const startIdx = content.indexOf(startMarker);
  const endIdx = content.indexOf(endMarker);
  if (startIdx !== -1 && endIdx !== -1) {
