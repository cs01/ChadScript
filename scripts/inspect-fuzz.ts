// Discovery runs of the console.log inspect fuzzer (the suite tests/slow/inspect-fuzz.test.ts runs
// a fixed 20-seed corpus). Each program is checked against Node at -O0/-O2 + IR verify; a
// divergence is written to .fuzz/inspect-fail-<seed>.ts as a ready-made repro.
//
//   bun run scripts/inspect-fuzz.ts --count 200 --seed 1

import { mkdirSync, writeFileSync } from "node:fs";
import { join, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { genInspectProgram } from "../tests/harness/inspect-gen.js";
...
for (let i = 0; i < count; i++) {
  ...
  let divergences;
  try {
    divergences = await differentialSource(program, `ifuzz${seed}`);
  } catch (e) {
    divergences = [{ kind: "error" as const, detail: (e as Error).message }];
  }
  if (divergences.length > 0) {
    failures++;
    const path = join(outDir, `inspect-fail-${seed}.ts`);
