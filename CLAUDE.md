...
3. **~~Never insert new optional fields in the MIDDLE of an interface~~** — retired. `src/semantic/interface-layout-normalizer.ts` now rewrites object literal properties to the interface's declared field order before codegen for `const x: I = {...}` declarations. Property source order no longer determines struct layout for interface-typed variables. You CAN now add optional fields anywhere in an interface. (Other sites — type assertions, return values, etc. — are still migrating one caller at a time. When in doubt, still declaration-order is safest.)
