...
export const DOD: DodItem[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      "run/string-conversion-objects.ts",
      "reject/string-conversion-function.ts",
      "reject/string-conversion-subclass-tostring.ts",
      "run/catch-error-message.ts",
      "run/error-classes.ts",
      "reject/class-extends-error.ts",
      "reject/error-stack.ts",
      "run/promise-constructor.ts",
      "run/promise-constructor-values.ts",
      "reject/promise-executor-unannotated.ts",
      "run/json-stringify-cycle.ts",
      "run/json-stringify-cycle-paths.ts",
      "run/console-format-json-cycle.ts",
      "run/map-ctor-entries.ts",
      "run/map-ctor-pairs.ts",
      "reject/map-ctor-variable.ts",
      "run/array-push-many.ts",
    ],
    note: "Every @known-bug fixture is cleared: implemented to match Node (string conversion of objects and arrays, the Error classes, new Promise with a typed executor, the JSON cycle TypeError, new Map from pair literals) or rejected at validate (a class extending Error). The CLI half (chad run --fallback=node, chad doctor, chad --version) is pinned by tests/slow/cli.test.ts. Released as the v0.1.0 tag; see CHANGELOG.md.",
  },
];
