...
async function runFixture(compiler: string, tc: TestCase, outDir: string): Promise<void> {
  ...
  try {
    ...
    if (isCrashSignal(err.signal)) {
      throw new Error(
        `Compiler crashed with ${err.signal} on ${tc.fixture}\nstderr: ${err.stderr?.slice(0, 1000) || ""}`,
      );
    }
    throw new Error(
      `Compilation failed for ${tc.fixture}\nstderr: ${err.stderr?.slice(0, 1000) || ""}\nstdout: ${err.stdout?.slice(0, 500) || ""}`,
    );
  }
  ...
  if (tc.expectTestPassed) {
    ...
    if (!stdout.includes("TEST_PASSED")) {
      throw new Error(
        `${tc.name}: expected TEST_PASSED in stdout, got: ${stdout.slice(0, 500)}\nstderr: ${(result.stderr || "").slice(0, 500)}`,
      );
    }
    assert.strictEqual(
      actualExitCode,
      0,
      `${tc.name}: expected exit code 0, got ${actualExitCode}`,
    );
  } else if (tc.expectedExitCode !== undefined) {
    assert.strictEqual(
      actualExitCode,
      tc.expectedExitCode,
      `${tc.name}: expected exit ${tc.expectedExitCode}, got ${actualExitCode}`,
    );
  } else {
...
describe("Self-Hosting", { timeout: 600000 }, () => {
  ...
  describe("Self-hosting chain", { timeout: 600000 }, () => {
    it("Node.js → Stage 0: compile chad-native.ts", async () => {
      ...
      assert.ok(
        stats.size > 100000,
        `Stage 0 binary should be substantial (got ${stats.size} bytes)`,
      );
    });
    ...
    it("Stage 0 smoke test: compile and run hello.ts", async () => {
      ...
      try {
        ...
        try {
          if (fsSync.existsSync(outBinary)) fsSync.unlinkSync(outBinary);
        } catch {}
      }
    });
    ...
    it("Stage 0 → Stage 1: Stage 0 compiles native-compiler.ts", async () => {
      ...
      assert.ok(
        stats.size > 100000,
        `Stage 1 binary should be substantial (got ${stats.size} bytes)`,
      );
    });
