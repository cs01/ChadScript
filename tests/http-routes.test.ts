...
describe("HTTP Route Isolation Tests", { concurrency: 1 }, () => {
  ...
  async function testRoute(
    ...
    try {
      ...
      if (opts?.bodyContains !== undefined) {
        assert.ok(
          body.includes(opts.bodyContains),
  ...
  it("GET /large with Accept-Encoding: deflate returns compressed response", async () => {
    ...
    try {
      const { headers, body } = await new Promise<{
      ...
      assert.strictEqual(
        ...
        "deflate",
        "Response should have Content-Encoding: deflate",
      );

      const decompressed = zlib.inflateSync(body);
      const text = decompressed.toString("utf-8");
      assert.ok(
        text.includes("Large Response"),
  ...
  it("GET /json with Accept-Encoding: deflate skips compression for small body", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        headers["content-encoding"],
        undefined,
        "Small response should NOT have Content-Encoding header",
      );
  ...
  it("GET /large without Accept-Encoding does not compress", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        headers["content-encoding"],
        undefined,
        "Response without Accept-Encoding: deflate should not be compressed",
      );
  ...
  it("GET /large with Accept-Encoding: zstd returns zstd compressed response", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        headers["content-encoding"],
        "zstd",
        "Response should have Content-Encoding: zstd",
      );
  ...
  it("GET /large with Accept-Encoding: zstd, deflate prefers zstd", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        headers["content-encoding"],
        "zstd",
        "zstd should be preferred over deflate",
      );
