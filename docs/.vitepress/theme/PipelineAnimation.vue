...
// The source and IR lines are real: this is what the compiler emits for this program at -O0
// (allocas and loads trimmed, temporaries renamed). Numbers are doubles, as in JavaScript.
const codeLines = [
  { text: "function fib(n: number): number {", color: "#c792ea" },
  { text: "  if (n <= 1) return n;", color: "#89ddff" },
  { text: "  return fib(n - 1) + fib(n - 2);", color: "#f78c6c" },
  { text: "}", color: "#c792ea" },
  { text: "console.log(fib(30));", color: "#82aaff" },
];
...
const blocks = [
  {
    ...
    ir: "define double @fib.1(double %n) {",
    left: 0,
    width: 100,
  },
  {
  ...
  },
  {
    ...
    ir: "  br i1 %c, label %then, label %end",
    left: 58,
    width: 42,
  },
  {
    ...
    ir: "  %a = call double @fib.1(double %n1)",
    left: 0,
    width: 38,
  },
  {
  ...
  },
  {
    ...
    line: 2,
    label: "CallExpression",
    ir: "  %b = call double @fib.1(double %n2)",
    left: 64,
    width: 36,
  },
  { id: 6, line: 3, label: "ReturnStatement", ir: "  ret double %s", left: 0, width: 100 },
  {
    id: 7,
    line: 4,
    label: "CallExpression",
    ir: "call void @cs_print_f64(double %r)",
    left: 0,
    width: 100,
  },
];
...
const stepDescriptions: Record<string, string> = {
  source: "Ordinary TypeScript, the same file Node runs",
  parse: "tsc parses and typechecks it at maximum strictness",
  analyze: "The default-deny validator admits each node or rejects it with a CS code",
  codegen: "Lowered to typed HIR, verified, then emitted as LLVM IR",
  link: "clang optimizes and links it with the runtime",
};
...
<template>
  <div class="pipeline-wrap" ref="containerRef">
      ...
      <div class="breadbar" :class="{ show: phase >= 1 }">
        <template v-for="(s, i) in steps" :key="s">
          ...
          <div
            ...
            class="crumb-line"
            :class="{ lit: steps.indexOf(step) > i }"
          ></div>
        </template>
      </div>
          ...
          <div class="cube-emerge" :class="{ show: phase >= 9, growing: phase === 8 }">
            <div class="cube-row" @click="replay" title="Click to replay">
              ...
              <div class="cube-info">
                ...
                <div class="cube-stats">{{ stats }}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="caption" :class="{ show: phase >= 11 }">
      No interpreter, no JIT, no engine in the binary: generated code plus a small runtime.
    </div>
  </div>
</template>
...
@keyframes cube-rotate {
  ...
  to {
    transform: rotateX(-25deg) rotateY(395deg);
  }
}
