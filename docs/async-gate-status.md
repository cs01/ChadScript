...
| **FIFO waiters** | ❌ **BUG: LIFO** | `cs_await` front-inserts (`w->next = p->waiters; p->waiters = w`), and `cs_promise_resolve` iterates that list — so multiple awaiters of one promise resume in **reverse** registration order. JS resumes them in registration order. Fix: append to a tail, or reverse on settle. |
