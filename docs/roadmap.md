...
- Prebuilt compiler binaries, and `--fallback=milojs`.
- Classes that extend `Error` or other built-in classes, and `.stack` on errors.
- `.then()` on promises, and `new Promise` with an executor that is not written inline.
