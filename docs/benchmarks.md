...
- **Matches C on matrix multiply** — 0.44s vs C's 0.45s on 512x512 dense matmul, faster than Go, 1.4x faster than Bun/Node
- **Faster than Go on recursion** — fib(42) in 1.6s vs Go's 1.81s, 2x faster than Bun, 3x faster than Node
- **1.7ms cold start** — within 13% of C, 2x faster than Go, 11x faster than Bun, 32x faster than Node
- **Near-C JSON** — 9ms to parse+stringify 10K objects via yyjson, 2.6x faster than Node, 3.7x faster than Go
